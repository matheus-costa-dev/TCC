...
Já o ponto de menor variância do portfólio é composto por `r toPercent(as.numeric(summarysafetyCombination))` alocado em `r textList(names(summarysafetyCombination))` respectivamente, com um índice Sharpe de `r round(safetyCombination$sharpeRatePortfolio,4)`, desvio padrão de `r safetyCombination$sd` e retorno esperado de `r safetyCombination$mean`.
